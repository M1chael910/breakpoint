...
class DataService {
    ...
    func createGroup(withTitle title: String, andDescription description: String, forUser ids: [String], handler: @escaping(_ groupCreated: Bool) ->()) {
        let groupValues: [String : Any] = ["title" : title, "description" : description, "members" : ids]
        REF_GROUPS.childByAutoId().updateChildValues(groupValues)
        handler(true)
    }
        ...
        REF_GROUPS.observeSingleEvent(of: .value) { (groupSnapshot) in
            ...
            for group in groupSnap {
                ...
                if memberArray.contains((Auth.auth().currentUser?.uid)!) {
                    ...
                    let group = Group(title: title, description: description, key: group.key, membersCount: memberArray.count, members: memberArray)
